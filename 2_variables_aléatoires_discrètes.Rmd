...
\noindent Soit $X$ une variable aléatoire réelle de **support fini** $R_X=\{x_1,\dots,x_n\}$. On appelle **espérance** de $X$ le nombre réel 
...
Pour une variable aléatoire réelle discrète de **support infini dénombrable** $R_x=\{x_1,\dots, x_n\dots\}$, l'existence d'une espérance n'est pas systématique. On dit qu'une telle variable $X$ admet une espérance si la série $\sum\limits_{n}\mathbb{P}(X=x_n)\,x_n$ est convergente. Dans ce cas, l'espérance $\mathbb{E}(X)$ de $X$ est la somme de cette série :
...
\noindent Cette expression de l'espérance s'écrit à partir des valeurs $x_k$ prises par $X:\Omega\longrightarrow\mathbb{R}$, c'est-à-dire les éléments de l'espace d'arrivée. Il est parfois plus commode d'utiliser une variante de cette formule, faisant plutôt intervenir les éléments de l'espace de départ $\Omega$ :

:::: {.defbox .def data-latex="important"}
<center>**Espérance d'une variable aléatoire réelle discrète (variante)**</center>
Soit $X:\Omega\longrightarrow\mathbb{R}$ une variable aléatoire réelle discrète. Alors, son espérance, si elle existe, est donnée par la formule
...
**Démonstration.** $X$ étant discrète, il existe $K\subset\mathbb{N}$ (fini ou non) tel que le support de $X$ s'écrive

$$R_X=\{x_k\,;\,k\in K\}$$

L'univers $\Omega$ peut alors se décomposer en 

$$\Omega=\bigsqcup\limits_{k\in K}\Omega_k$$
\noindent où $\Omega_k=X^{-1}(\{x_k\})=(X=x_k)$.

\noindent On a donc, sous réserve d'existence :

\begin{align}
\sum\limits_{\omega\in\Omega}\mathbb{P}(\{\omega\})X(\omega)&=\sum\limits_{k\in K}\sum\limits_{\omega\in\Omega_k}\mathbb{P}(\{\omega\})X(\omega) \\
&=\sum\limits_{k\in K}\sum\limits_{\omega\in\Omega_k}\mathbb{P}(\{\omega\})x_k \\
&=\sum\limits_{k\in K}x_k\sum\limits_{\omega\in\Omega_k}\mathbb{P}(\{\omega\}) \\
&=\sum\limits_{k\in K}x_k\,\mathbb{P}(\Omega_k) \\
...
:::: {.thmbox .def data-latex="important"}
**Théorème de transfert.** Soient $X:\Omega\longrightarrow\mathbb{R}$ une variable aléatoire réelle admettant une espérance et $\phi:X(\Omega)\longrightarrow\mathbb{R}$ une fonction. Alors, sous réserve d'existence de $\mathbb{E}(\phi(X))$, on a 

$$\mathbb{E}(\phi(X))=\sum\limits_{k\in K}\mathbb{P}(X=x_k)\,\phi(x_k)$$
\noindent En particulier, pour $\phi(x)=x^m$, avec $m$ entier naturel, on a donc

$$\mathbb{E}(X^m)=\sum\limits_{k\in K}\mathbb{P}(X=x_k)\,x_k^m$$
::::
...
&=\sum\limits_{k\in K}\phi(x_k)\sum\limits_{\omega\in X^{-1}(x_k)}\mathbb{P}(\{\omega\}) \\
&=\sum\limits_{k\in K}\mathbb{P}(X=x_k)\phi(x_k) \\
\end{align}
...
\noindent Avec ce qui précède, la série $\sum\limits_{n}\mathbb{P}(X=n)n=\sum\limits_{n=1}^{\infty}(1-p)\,n\,p^{n-1}$ est convergente, de somme $\frac{1-p}{(1-p)^2}=\frac{1}{1-p}$. Autrement dit, $X$ admet une espérance et $\mathbb{E}(X)=\frac{1}{p}$.
...
\noindent Ainsi, $p_i$ ne dépend pas de $i$ : $p_i=p$, et donc 
...
\noindent Soient $X$ une variable aléatoire et $r\in\mathbb{N}$. On appelle **moment d'ordre $r$**, l'espérance - si elle existe - de $X^r$ : 

$$m_r=\mathbb{E}(X^r)$$
\noindent De même, sous réserve d'existence, le **moment centré d'ordre $r$** est le réel

$$\mu_r=\mathbb{E}\left((X-\mu)^r\right)$$
\noindent où $\mu=\mathbb{E}(X)$.

\noindent 

**Cas $r=1$ et $r=2$ (toujours sous résevre d'existence) :**
