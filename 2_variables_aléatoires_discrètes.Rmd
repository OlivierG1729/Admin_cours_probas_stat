...
<center>**Variables aléatoires réelles, variables aléatoires discrètes**</center>
Soit $(\Omega, \mathcal{A}, \mathbb{P})$ un espace probabilisé. On appelle **variable aléatoire réelle** toute application

$$X:\Omega\longrightarrow\mathbb{R}$$
\noindent telle que, pour tout intervalle ouvert $I$ de $\mathbb{R}$, l'ensemble 

$$X^{-1}(I)=\{\omega\in\Omega, X(\omega)\in I\}$$
\noindent est un élément de la tribu $\mathcal{A}$. En particulier, si $\mathcal{A}=\mathcal{P}(\Omega)$, toute application $X:\Omega\longrightarrow\mathbb{R}$ est donc une variable alétoire réelle.

\noindent On dira de plus que $X$ est une **variable aléatoire discrète** si son ensemble image 

$$X(\Omega)=\left\{X(\omega),\,\omega\in\Omega\right\}\subset\mathbb{R}$$
\noindent est soit fini, soit infini mais dénombrable (ce que l'on résume souvent en *au plus dénombrable*).
::::

\noindent

**Remarques. i.** En pratique dans ce cours, et pour rester dans les contours du programme, nous ne vérifierons jamais (sauf si cela est explicitement demandé lors d'un exercice) que $X$ est bien une variable aléatoire, autrement dit nous supposerons toujours la propriété $X^{-1}(I)\in\mathcal{A}$ acquise (ce qu'elle n'est pas dans le cas général). Cette propriété est en particulier toujours vraie si on suppose que $\mathcal{A}=\mathbb{P}(\Omega)$, ce qui sera souvent sous-entendu dans les exercices (notamment dans le cas discret). 

<center>**On retiendra donc surtout la définition pratique donnée un peu plus bas**</center>

\noindent 

**ii. (Hors programme).** Dans le cas général, on peut définir une notion de variable aléatoire entre deux espaces **probabilisables** (la définition en elle-même ne suppose pas l'existence d'une mesure de probabilité, ni dans l'espace de départ ni dans l'espace d'arrivée) en disant que pour $(\Omega, \mathcal{A})$ et $(\Omega', \mathcal{A}')$ deux espaces probabilisables, une application 

$$X:(\Omega, \mathcal{A})\longrightarrow (\Omega', \mathcal{A}')$$
est une variable aléatoire lorsque

$$\forall B\in\mathcal{A}', \, X^{-1}(B)=\{\omega\in\Omega, X(\omega)\in\mathcal{A}'\}\in \mathcal{A}$$
\noindent autrement dit, lorsque l'image réciproque d'un observable (un élément de $\mathcal{A'}$) est encore un observable (un élément de $\mathcal{A}$).

\noindent Dans la définition qui est encadrée, la tribu de l'espace d'arrivée est implicitement égale à la tribu *borélienne* sur $\mathbb{R}$, autrement dit la tribu de tous les ensembles pouvant s'écrire comme combinaison d'ouverts via les opérations de réunion dénombrable, d'intersection dénombrable et de passage à l'événement contraire. Cette tribu est notée $\mathcal{B}(\mathbb{R})$. Une telle tribu est dite *engendrée* par les ouverts de $\mathbb{R}$, et il s'agit de la plus petite tribu (au sens de l'inclusion) sur $\mathbb{R}$ contenant tous les ouverts de $\mathbb{R}$. Ses éléments sont appelés des *boréliens*. Dans ce cas, une variable aléatoire à  valeurs dans $\mathbb{R}$ est une application vérifiant $X^{-1}(B)\in\mathcal{A}$ pour tout $B$ dans $\mathcal{B}(\mathbb{R})$. On peut alors démontrer qu'il suffit en fait d'exiger cette propriété pour tout intervalle ouvert $B$ pour avoir une variable aléatoire, autrement dit dès que cette propriété est vraie pour les intervalles ouverts elle l'est plus généralement pour tous les boréliens.

\noindent 

:::: {.defbox .def data-latex="important"}
<center>**Variables aléatoires discrètes en pratique**</center>

\noindent Dans le cadre du programme de ce concours, une variable aléatoire discrète est, sauf mention explicite du contaire, une application 

$$X:\Omega\longrightarrow F$$
\noindent où $\Omega$ est l'univers d'une expérience aléatoire et $F$ est un ensemble au plus dénombrable.
::::
...
\noindent Comme dit précédemment, la notion de variable aléatoire dans son sens le plus général se fait entre deux espaces probabilisables. Mais si l'on souhaite donner un sens à la notion de *loi d'une variable* aléatoire, on est obligé de prendre comme espace de départ un espace probabilisé $(\Omega, \mathcal{A}, \mathbb{P})$ Sous cette condition, on peut définir une loi sur l'espace d'arrivée, appelée *loi image de $\mathbb{P}$ par $X$* et notée $\mathbb{P}_X$. Cette loi est appelée aussi plus simplement la *loi de $X$*.

:::: {.thmbox .def data-latex="important"}
**Théorème (Loi d'une variable aléatoire discrète).** Soient $(\Omega, \mathcal{A}, \mathbb{P})$ un espace probabilisé et $X$ une variable aléatoire discrète définie sur $\Omega$ et à valeurs dans $\Omega'=X(\Omega)$. On peut définir une loi de probabilité $\mathbb{P}_X$ sur $(\Omega', \mathcal{P}(\Omega'))$, appelée **loi de $X$** en posant, pour tout $i$ dans $\Omega'$ :

$$\mathbb{P}_X(\{i\})=\mathbb{P}(X^{-1}(\{i\}))$$
\noindent On notera plus simplement :

- $(X=i)$ l'événement $X^{-1}(\{i\})$  ;
- $\mathbb{P}_X(i)=\mathbb{P}_X(\{i\})$.

\noindent La loi de $X$ est donc définie par :

- la donnée de $X(\Omega)$, i.e. l'ensemble de toutes les valeurs prises par $X$ ;
- la donnée, pour tout $i$ dans $X(\Omega)$, de la probabilité $\mathbb{P}(X=i)$.
...
**Remarque.** Si $X$ prend *a priori* les valeurs $(x_i)_I$, la loi de $X$ est donc définie par la donnée de tous les $x_i$ et de toutes les probabilités $\mathbb{P}(X=i)$. Toutefois il peut arriver qu'*a posteriori*, c'est-à-dire après calculs, on se rende compte que pour certains indices $i_0$ de $I$ l'on ait $\mathbb{P}(X=i_0)=0$. Dans ce cas, on peut très bien appeler loi de $X$ la donnée des couples $(j, \mathbb{P}(X=j))$ pour toutes les valeurs $j$ de $I$ telles que $\mathbb{P}(X=j)>0$. La loi de $X$ n'est donc pas unique au sens strict du terme, mais elle l'est bien à des événements élémentaires de probabilité nulle près. Par exemple, dans l'exercice 2 du sujet du concours interne de 2012 (exercice 2.7. de ce cours) la variable aléatoire $X$ prend *a priori* les valeurs $0,1,2,3$, mais *a posteriori* on a $\mathbb{P}(X=0)=0$, donc on peut enlever $0$ de $X(\Omega)$.
...
:::: {.thmbox .def data-latex="important"}
**Théorème (calcul de $\mathbb{P}(X\in B)$).** Pour $X:\Omega\longrightarrow F$ une variable aléatoire discrète et $B\in\mathcal{P}(X(\Omega))$, on calcule la probablité $\mathbb{P}(X\in B)$ par

$$\mathbb{P}(X\in B)=\mathbb{P}\left(\bigcup\limits_{n\in B} (X=n)\right)=\sum\limits_{n\in B}\mathbb{P}(X=n)$$
::::

\noindent

**Démonstration.** Les événements $(X=n), n\in B$ forment une partition de l'événement $(X\in B)$. On conclut en utilisant le fait qu'une probabilité d'une réunion disjointe d'événements $A_n$ est la somme des probablités $\mathbb{P}(A_n)$.
...
\noindent La fonction de répartition d'une variable aléatoire discrète (ou même continue comme on le verra dans le prochain chapitre) est définie à partir des événements du type $(X\leq x)$ :

:::: {.defbox .def data-latex="important"}
<center>**Fonction de répartition**</center>

\noindent Soit $X$ une variable aléatoire discrète. On appelle **fonction de répartition de $X$** la fonction $F_X$ définie sur $\mathbb{R}$ par 
...
Soit $F$ la fonction de répartition d'une loi discrète. Alors :
...
**Démonstration.** On note $X$ une variable aléatoire discrète ayant $F$ pour fonction de répartition. 
...
**ii.** Soit $x$ un réel. Les intervalles $\left]-\infty\,;\,x+\frac{1}{n}\right]$ forment une suite décroissante (par rapport à $n$) pour l'inclusion, donc d'après la propriété de continuité décroissante des probabilités, on a 

$$\mathbb{P}\left(\bigcap_{n=0}^{\infty}\left]-\infty\,;\,x+\frac{1}{n}\right]\right)=\lim\limits_{n\to\infty}\mathbb{P}\left(\left]-\infty\,;\,x+\frac{1}{n}\right]\right)$$
