...
*- éviter les redites afin de gagner un peu de temps, sans trop sacrifier le point précédent. Sur un type de raisonnement que l'on a déjà rédigé de façon rigoureuse lors d'une question précédente, on peut probablement aller un peu plus vite sans s'attirer les foudres de la personne qui corrige la copie...*
...
$$\forall (i,j)\mathbb{N}^2, \, \mathbb{P}(X=i, Y=j)=\frac{(i+j)\lambda^{i+j}}{e\,i!\,j!}$$
...
:::: {.methbox .meth data-latex="important"}
**Exercice 3.10.** Soit $X$ une variable aléatoire suivant une loi exponentielle de paramètre $\frac{1}{2}$.
...
:::: {.methbox .meth data-latex="important"}
**Exercice 3.11.** Soient $X$ et $U$ deux variables aléatoires indépendantes, où $X\sim\mathcal{N}(0,1)$ et $U\sim\mathcal{U}_{\{-1,1\}}$. On définit $Y=UX$.
...
:::: {.methbox .meth data-latex="important"}
**Exercice 3.12. 1.** Soient $X$ et $Y$ deux variables aléatoires indépendantes de loi commune $\mathcal{E}(\lambda)$, avec $\lambda>0$. Déterminer une densité de $X+Y$.
...
:::: {.methbox .meth data-latex="important"}
**Exercice 3.13.** Soient $X$ et $Y$ deux variables indépendantes de loi normale centrée réduite (de densité $\varphi$ et de fonction de répartition $\Phi$). On pose
...
:::: {.methbox .meth data-latex="important"}
**Exercice 3.14.** Soit $(X,Y)$ un couple de variables aléatoires, de densité 
